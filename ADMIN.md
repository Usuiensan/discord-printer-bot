...
推奨構成は Proxmox 上で役割を分けます。

- Debian 13 standard コンテナ/VM: Discord bot 本体を常駐
- Windows VM: Epson ドライバ、OPOS ADK for .NET、印刷ブリッジのみを常駐
- Epson TM-T70II: Windows VM に USB パススルー

この構成では Debian 側が Discord から投稿を受けて ESC/POS データを生成し、Windows VM の印刷ブリッジへ HTTP で送ります。Windows VM は OPOS 状態確認と Windows プリンタキューへの送信だけを担当します。

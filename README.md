...
指定した1つ以上のチャンネルに投稿すると、Epson TM-T70II 58mm レシートプリンタへ印刷されます。
...
画像化文字の行間は`TEXT_IMAGE_LINE_HEIGHT_DOTS + TEXT_IMAGE_LINE_GAP_DOTS`です。既定値では36ドットになります。
...
管理者がローカルLLM連携を有効にしている場合、Botへメンションすると印刷ではなく高速AIチャット（no-think）として処理されます。じっくり考えさせる場合は、質問の先頭に`/think`を付けます。`/fast`で高速回答を明示することもできます。

```text
@Bot 今日の作業を3つに整理して
@Bot /think この設計の問題点を詳しく検討して
@Bot /fast 1+1は？
```

処理中は質問に🤔リアクションが付き、完了すると質問者へのメンション付きで回答します。`/think`では思考の直近部分と経過時間を1件のメッセージ上で更新し、「中止」「高速回答に切替」ボタンを表示します。高速回答後の「深く考え直す」ボタンから同じ質問を`/think`で再実行できます。

回答末尾には思考時間、実際に応答したモデル名、Ollamaが返した詳細な思考過程のテキストファイルへのリンクが付きます。モデルが詳細思考を返さない場合、テキストファイルにその旨を記録します。

会話履歴はチャンネル単位でBotのメモリに保持され、Botを再起動すると消去されます。
...
文章を指定dots幅で折り返し、画像文字として印刷できます。コマンド行と通常のDiscordヘッダーは印刷されません。
...
`360`は回転前の1行の長さ360dotsです。各行を積み上げた高さが印字可能幅384dotsを超える直前で画像を分割し、それぞれ90度回転して連続印刷してからカットします。
...
指定幅が384dots以下ならその幅で折り返します。384dotsを超える値を指定した場合は384dotsへ丸め、通常の紙幅画像として印刷します。指定できる範囲は1～4096dotsです。

`!preview`を先頭に付けると、分割後の印刷画像を中央揃えで連結したプレビューを確認できます。
...
- `|`: 列区切り
- `{width:*,10}` / `{w:*,10}`: 列幅
- `{border:space|none|line|2}` / `{b:...}`: 列間
- `{align:left|center|right}` / `{a:...}`: 行揃え
- `{text:wrap|nowrap}` / `{t:...}`: 折り返し
- `-` だけの行: 水平線
- `=` だけの行: その場でカット
- `_下線_`, `"強調"`, `` `反転` ``, `^倍角^`: 文字装飾
- `{code:...; option:qrcode}`: QR/バーコード
- `{image:...}`: PNG base64 画像

ReceiptLine の完全な仕様のうち、印刷でよく使う範囲から対応しています。未知のプロパティと `{command:...}` は無視します。
...
- `!cut` は文字がカッター位置へ掛からないよう、既定で前後3行を送ってからその場でカットします。空行数は`CUT_FEED_LINES`で変更できます。

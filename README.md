...
管理者がローカルLLM連携を有効にしている場合、Botへメンションすると印刷ではなくAIチャットとして処理されます。

```text
@Bot 今日の作業を3つに整理して
```

処理中は質問に🤔リアクションが付き、完了すると質問者へのメンション付きで回答します。回答末尾には思考時間、実際に応答したモデル名、Ollamaが返した詳細な思考過程のテキストファイルへのリンクが付きます。モデルが詳細思考を返さない場合、テキストファイルにその旨を記録します。
